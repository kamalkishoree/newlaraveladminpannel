	...
	<div class="page-header">
		<div class="card breadcrumb-card">
			<div class="row justify-content-between align-content-between" style="height: 100%;">
				<div class="col-md-6">
					<h3 class="page-title">{{__('Financial Category Details')}}</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item">
							<a href="{{ route('dashboard') }}">Dashboard</a>
						</li>
						<li class="breadcrumb-item active-breadcrumb">
							<a href="{{ route('financial-category.index') }}">{{ __('Financial Category') }}</a>
						</li>
					</ul>
				</div>
                    <div class="col-md-3">
                        <div class="create-btn pull-right">
                            <a href="{{ route('financial-category.create') }}" class="btn custom-create-btn">{{ __('Add New Financial Category') }}</a>
                        </div>                 
                    </div>
			</div>
		</div><!-- /card finish -->	
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <form  action="{{ route('financial-category.index') }}" method="GET" enctype="multipart/form-data" >
...
@push('scripts')
    <script>
        $(function() {
            $('#table').DataTable({
              ...
              ajax: {
                url: "{{ route('financial-category.index') }}",
                type: "GET",
                    data: function(d) {
    ...
    <script type="text/javascript">
        $("body").on("click", ".remove-financial-category", function(e) {
            e.preventDefault();
    ...
    <script type="text/javascript">
        function changeUserStatus(_this, id) {
            ...
            $.ajax({
                url: `{{ route('financial-category.status_update') }}`,
                type: 'GET',
                data: {
                    _token: _token,
                    id: id,
